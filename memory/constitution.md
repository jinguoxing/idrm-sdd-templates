...
| **字段注释** | `COMMENT '...'` | 建表时**每个字段**必须包含详细注释 |
...
采用 **6 位数字** 格式：`XXX` (服务) + `YYY` (模块/功能)。

| 范围 | 类型 | 说明 |
|------|------|------|
| **100001 - 100999** | 系统错误 | 框架/基础设施级错误 (如 DB 连接失败, 序列化错误) |
| **200000 - 999999** | 业务错误 | 按服务和模块分配 |

### 业务错误码分配

业务错误码由各服务负责人定义，需在 `api/internal/types/errors.go` (或 `vars.go`) 中维护。
